...
use gl::types;
use glfw::{fail_on_errors, Context, Glfw, GlfwReceiver, InitError, PWindow, WindowMode};
use log::info;
...
impl LinuxWindow {
        ...
        let (mut glfw_window_handle, glfw_event_handle) = 
        match glfw_handle.create_window(props.width, props.height, &props.title, WindowMode::Windowed) {
            Some(ret) => ret,
...
impl WindowInterface for LinuxWindow {
    fn on_update(&mut self) {
        self.glfw_window_handle.set_key_polling(true);
